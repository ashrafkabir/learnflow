...
/**
 * Iteration 56 — Screenshot capture for every major screen/state.
 * Saves into repo: learnflow/screenshots/iter56
 * And OneDrive: /home/aifactory/onedrive-learnflow/evals/screenshots/iter56
 */

const OUT_REPO = '/home/aifactory/.openclaw/workspace/learnflow/screenshots/iter56';
const OUT_OD =
  process.env.LEARNFLOW_E2E_OUT_OD || '/home/aifactory/onedrive-learnflow/evals/screenshots/iter56';
...
test.describe('Iter56 screenshots (client app)', () => {
  ...
  test('onboarding: welcome -> goals -> topics -> api keys -> subscription -> first course', async ({
    ...
    await page.addInitScript(() => {
      // Force deterministic access to onboarding screens
  ...
  test('core: dashboard, conversation, mindmap, marketplace, settings', async ({ page }) => {
    await page.addInitScript(() => {
      (window as any).__LEARNFLOW_ENV__ = { VITE_DEV_AUTH_BYPASS: '1' };
      // Clear auth to prove bypass works
      localStorage.removeItem('learnflow-token');
      localStorage.removeItem('learnflow-user');
      localStorage.removeItem('learnflow-onboarding-complete');
    });
