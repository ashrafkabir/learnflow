...
export function QualityCheckList({ results }: { results: QualityCheck[] }) {
  if (!results.length) return <p className="text-gray-400 text-center py-2">Running checks...</p>;

  return (
    <div className="space-y-1.5">
      {results.map(r => (
        <div key={r.lessonId} className="bg-white dark:bg-gray-900 rounded-lg px-2 py-1.5 border border-gray-100 dark:border-gray-700">
          ...
          <div className="flex gap-2 mt-1 pl-5">
            ...
            <CheckBadge pass={r.checks.objectives.pass} label={`${r.checks.objectives.count} obj`} />
