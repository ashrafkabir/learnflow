    ...
    <MarketingLayout>
      ...
      <section className="max-w-4xl mx-auto px-6 py-20">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 mb-16">
          {PLATFORMS.map((p) => (
            <div key={p.name} className="p-6 rounded-2xl border border-gray-100 dark:border-gray-800 hover:border-accent/30 hover:shadow-lg transition-all text-center">
              ...
              <div className="space-y-2">
                {p.formats.map((f) => (
                  <button key={f} className="w-full py-2 px-4 bg-accent text-white rounded-lg text-sm font-medium hover:bg-accent-dark transition-colors">
                    {f}
                  </button>
                ))}
              </div>
