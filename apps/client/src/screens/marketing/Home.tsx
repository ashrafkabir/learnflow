...
const DEMO_STEPS = [
  { icon: '💬', title: 'Tell us your goal', desc: 'Type any topic — "Learn Rust programming" or "Understand quantum mechanics." Our AI listens.', color: 'from-blue-500 to-cyan-400' },
  { icon: '⚙️', title: 'AI builds your course', desc: 'Multiple agents research the web, curate content, build lessons, and generate quizzes — in minutes.', color: 'from-violet-500 to-purple-400' },
  { icon: '🏆', title: 'Learn and master', desc: 'Study at your pace with adaptive quizzes, smart notes, and a mindmap that grows with your knowledge.', color: 'from-emerald-500 to-teal-400' },
];
  ...
  return (
    <MarketingLayout>
      ...
      <section className="relative overflow-hidden">
        ...
        <KnowledgeGraphBg />
        <motion.div className="relative max-w-6xl mx-auto px-6 py-24 md:py-32 text-center" initial="hidden" animate="visible" variants={stagger}>
          <motion.div variants={fadeUp} className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-accent/20 text-accent text-sm font-semibold mb-6 border border-accent/30">
            <span>✨</span> Now with multi-agent AI pipeline
          </motion.div>
          <motion.h1 variants={fadeUp} className="text-4xl sm:text-5xl md:text-6xl font-extrabold tracking-tight leading-tight mb-6 text-gray-900 dark:text-white">
            Learn anything.<br />
            <span className="text-accent">Master everything.</span><br />
            Powered by AI agents.
          </motion.h1>
          <motion.p variants={fadeUp} className="text-lg sm:text-xl text-gray-700 dark:text-gray-300 max-w-2xl mx-auto mb-10 leading-relaxed">
      ...
      {/* Social proof */}
      <motion.section className="border-y border-gray-100 dark:border-gray-800 bg-gray-50/50 dark:bg-gray-900/50" initial="hidden" whileInView="visible" viewport={{ once: true }} variants={stagger}>
        <div className="max-w-6xl mx-auto px-6 py-12 grid grid-cols-2 md:grid-cols-4 gap-8">
          {STATS.map((s) => (
            <motion.div key={s.label} className="text-center" variants={fadeUp}>
              ...
              <div className="text-sm text-gray-600 dark:text-gray-300 mt-1 font-medium">{s.label}</div>
            </motion.div>
          ))}
        </div>
      </motion.section>
      ...
      <section id="demo-section" className="max-w-6xl mx-auto px-6 py-20">
        <motion.div className="text-center mb-12" initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp}>
          ...
          <p className="text-gray-600 dark:text-gray-300 max-w-xl mx-auto">From idea to mastery in three simple steps.</p>
        </motion.div>
        <motion.div className="grid grid-cols-1 md:grid-cols-3 gap-8" initial="hidden" whileInView="visible" viewport={{ once: true }} variants={stagger}>
          {DEMO_STEPS.map((step, i) => (
            <motion.div key={step.title} variants={fadeUp} className="relative text-center group">
              <div className={`w-20 h-20 rounded-2xl bg-gradient-to-br ${step.color} flex items-center justify-center text-3xl mx-auto mb-4 shadow-elevated group-hover:scale-110 transition-transform duration-300`}>
                {step.icon}
              </div>
      ...
      <section className="max-w-6xl mx-auto px-6 py-20">
        <motion.div className="text-center mb-12" initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp}>
          ...
          <p className="text-gray-600 dark:text-gray-300 max-w-xl mx-auto">AI agents work together to create, teach, test, and track your learning journey.</p>
        </motion.div>
        <motion.div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6" initial="hidden" whileInView="visible" viewport={{ once: true }} variants={stagger}>
          {FEATURES.map((f) => (
            <motion.div key={f.title} variants={fadeUp} className="p-6 rounded-2xl border border-gray-100 dark:border-gray-800 hover:border-accent/30 hover:shadow-card transition-all group">
      ...
      <section className="max-w-6xl mx-auto px-6 py-20">
        ...
        <motion.div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12" initial="hidden" whileInView="visible" viewport={{ once: true }} variants={stagger}>
          {[
            ...
            <motion.div key={t.name} variants={fadeUp} className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-100 dark:border-gray-800 p-6 shadow-card">
        ...
        <div className="flex flex-wrap justify-center gap-6 mb-12">
          {[
            ...
            <div key={b.label} className="flex items-center gap-2 px-4 py-2 rounded-full bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 text-sm text-gray-600 dark:text-gray-300">
              <span>{b.icon}</span>
              <span>{b.label}</span>
            </div>
          ))}
      ...
      {/* CTA */}
      <motion.section className="max-w-6xl mx-auto px-6 py-20 text-center" initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp}>
        <h2 className="text-3xl font-bold mb-4">Ready to learn smarter?</h2>
        <p className="text-gray-600 dark:text-gray-300 mb-8 max-w-lg mx-auto">Join thousands of learners using AI to master new topics faster than ever.</p>
