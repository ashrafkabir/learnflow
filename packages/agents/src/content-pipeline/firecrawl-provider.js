...
\n${sourcePack}`;
    ...
    sections.push(`- By the end, you’ll be able to explain the core ideas behind ${topic} in plain English\n` +
    ...
    for (const s of sources.slice(0, 4)) {
        for (const kw of collectKeywords(s.content))
            topThemes.set(kw, (topThemes.get(kw) || 0) + 1);
    }
    const themes = [...topThemes.entries()]
        .sort((a, b) => b[1] - a[1])
        .slice(0, 8)
        .map(([w]) => w);
    sections.push(`### A simple mental model\n` +
    ...
    sections.push(`A good way to learn this is to map **inputs → process → outputs**. For example:\n\n` +
        '```\n' +
    ...
    sections.push(`Start with a tiny scenario and ask: what would change if we tweaked one assumption? ` +
    ...
    sections.push(`Even in a "foundations" lesson, it helps to know what’s still moving. A few frontier questions to explore next:\n` +
 ...
 */
export async function searchForLesson(courseTopic, moduleTitle, lessonTitle, lessonDescription, config = {}) {
    const cfg = { ...getDefaultConfig(), ...config };
