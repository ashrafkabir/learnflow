...
export async function wikipediaSummary(titleOrTopic) {
    try {
...
export async function arxivSearch(query, maxResults = 5) {
    try {
        ...
        console.warn('[WebSearch] arXiv search failed:', err.message);
        return [];
    }
}
export async function githubRepoSearch(query, maxResults = 5) {
    try {
        ...
        console.warn('[WebSearch] GitHub repo search failed:', err.message);
        return [];
    }
}
function tryDeriveGithubReadmeRaw(repoUrl) {
    try {
...
export async function scrapeWithReadability(url) {
    ...
    try {
        ...
        console.warn(`[WebSearch] Scrape failed for ${url}:`, err.message);
        return { content: '', title: '' };
    }
    finally {
        await sleep(120);
...
export async function fetchAndScoreSources(urls, topic, maxSources = 8) {
    const sources = [];
...
// ─── New Sources ─────────────────────────────────────────────────────────────
export async function redditSearch(query, limit = 5) {
    try {
...
export async function devtoSearch(query, limit = 5) {
    try {
        ...
        console.warn('[WebSearch] Dev.to search failed:', err.message);
        return [];
    }
}
export async function hackerNewsSearch(query, limit = 5) {
    try {
        ...
        console.warn('[WebSearch] Hacker News search failed:', err.message);
        return [];
    }
}
async function duckDuckGoSiteSearch(site, query, limit = 5) {
    try {
        ...
        while ((m = urlRegex.exec(html)) !== null && results.length < limit) {
            let u = m[0];
            u = u.split('&')[0];
            if (seen.has(u) || u.includes('duckduckgo.com'))
                continue;
            seen.add(u);
        ...
        console.warn(`[WebSearch] DuckDuckGo site:${site} search failed:`, err.message);
        return [];
    }
}
async function googleSiteSearch(site, query, limit = 5) {
    try {
        ...
        while ((m = urlRegex.exec(html)) !== null && results.length < limit) {
            ...
            u = u.split('&')[0];
            if (seen.has(u) || u.includes('google.com') || u.includes('webcache'))
                continue;
            seen.add(u);
            results.push({ url: u, title: `${site} article`, description: '' });
        }
        ...
        console.warn(`[WebSearch] Google site:${site} search failed:`, err.message, '— trying DuckDuckGo');
        return duckDuckGoSiteSearch(site, query, limit);
    }
}
export async function mediumSearch(query, limit = 3) {
    return googleSiteSearch('medium.com', query, limit);
}
export async function substackSearch(query, limit = 3) {
    return googleSiteSearch('substack.com', query, limit);
}
export async function quoraSearch(query, limit = 3) {
    return googleSiteSearch('quora.com', query, limit);
}
export async function theNewStackSearch(query, limit = 3) {
    return googleSiteSearch('thenewstack.io', query, limit);
}
// --- Additional Sources ---
export async function stackOverflowSearch(query, limit = 5) {
    try {
        ...
        console.warn('[WebSearch] StackOverflow search failed:', err.message);
        return [];
    }
}
export async function freeCodeCampSearch(query, limit = 3) {
    return googleSiteSearch('freecodecamp.org', query, limit);
}
export async function towardsDataScienceSearch(query, limit = 3) {
    return googleSiteSearch('towardsdatascience.com', query, limit);
}
export async function digitalOceanSearch(query, limit = 3) {
    return googleSiteSearch('digitalocean.com/community/tutorials', query, limit);
}
export async function mdnSearch(query, limit = 3) {
    return googleSiteSearch('developer.mozilla.org', query, limit);
}
export async function smashingMagSearch(query, limit = 3) {
    return googleSiteSearch('smashingmagazine.com', query, limit);
}
export async function courseraSearch(query, limit = 3) {
    return googleSiteSearch('coursera.org', query, limit);
}
export async function baiduScholarSearch(query, limit = 3) {
    return duckDuckGoSiteSearch('scholar.google.com', query, limit);
}
//# sourceMappingURL=source-fetchers.js.map