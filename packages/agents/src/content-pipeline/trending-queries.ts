...
function normalizeQueries(queries: unknown, min: number, max: number): string[] {
...
function heuristicTrendingQueries(topic: string): string[] {
...
export function createOpenAIQueryGenerator(env = process.env): QueryGenerator {
  ...
  return {
    async generateTrendingQueries(topic: string): Promise<string[]> {
      if (!openai) return heuristicTrendingQueries(topic);
        ...
        messages: [
          {
            role: 'system',
            content:
              'You generate web search queries for researching a course topic. Return JSON: {"queries": [..]}.\n' +
    ...
    async generateLessonQueries(args): Promise<string[]> {
      if (!openai) return heuristicLessonQueries(args);
        ...
        messages: [
          {
            role: 'system',
            content:
              'You generate web search queries for researching a specific lesson within a course. Return JSON: {"queries": [..]}.\n' +
